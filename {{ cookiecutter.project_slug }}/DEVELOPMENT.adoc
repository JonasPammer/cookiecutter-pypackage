...
In Addition:
When you alter the dependencies of the package
you need to re-run the command to generate the `requirements.txt` file:

[subs="+quotes,attributes"]
----
(venv)$ *pip-compile requirements.txt*
----
====

[NOTE]
====
When you alter the `requirements-*.in` files you need to re-run
the command which generates their `.txt` counterpart.
